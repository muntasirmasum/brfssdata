...
- Added a package citation (`citation("brfssdata")`) alongside the CDC
  data citation.
- Declared the minimum duckdb version (\>= 1.5.5). Connections are
  opened with `shared_home = FALSE`, which earlier duckdb releases
  reject.
- brfss_design() is much faster on 2001 and later years, and pooling
  ...
  errors, and degrees of freedom unchanged. A weighted mean over one
  year went from about 80 seconds to 1.5, and over two pooled years from
  345 seconds to 3. Pooling three years previously failed outright,
  because the nested-clusters check that `survey` runs on a design
  without nesting builds a cluster-by-stratum table too large for R to
  allocate.
