...
#'
#' @param years Optional integer vector of survey years to cite.
#'   `NULL` cites the collection's span as a single entry.
#'
...
brfss_citation <- function(years = NULL) {
  ...
  data_entries <- if (is.null(years)) {
    ...
    do.call(c, lapply(as.character(years), cdc))
  }
