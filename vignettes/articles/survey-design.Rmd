...
only signal is an upstream `NaNs produced` warning from `qt()`. A cell
whose respondents all gave the same answer reports a standard error of
0 on top of that, which is a property of the cell and not a claim of
perfect precision. `degf()` on the filtered design and the
`unweighted(n())` column in the summary are what catch this before it
reaches a table. Cells this thin belong in a report as counts, or
suppressed, which is how CDC handles small cells in its own
publications.
