...
The files carry no upstream provenance yet. `brfss_year_info()` reports
what the hosted copy contains, but not the CDC URL it was built from,
when it was retrieved, or the hash of the upstream Transport file, so
there is no way from inside the package to tell which CDC revision of a
year you are holding. Those columns are planned for the next data
republish.
