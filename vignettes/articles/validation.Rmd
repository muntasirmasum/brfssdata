...
`svyciprop()` defaults to `na.rm = FALSE`, and one `NA` anywhere in the
column carries through the whole calculation, so the estimate and both
limits come back `NA` with no warning to say why. `survey_mean()` takes
the same argument for the same reason.

Compare the result with the symmetric interval from `survey_mean()` for
the same quantity; for prevalences under about five percent, or any
...
which this package sets to `"adjust"` when the design contains a
single-PSU stratum), and the per-year data citation,
