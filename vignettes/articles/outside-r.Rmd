...
Three companion assets sit under a single `data-meta` tag. `manifest.json`
lists the published years, `brfss_variables.parquet` is the variable
catalog (2,128 distinct variables with their labels and the years each
one appears in), and `brfss_labels.parquet` is the value-label catalog
covering 1998 onward.
...
`options(survey.lonely.psu = "adjust")` when you have not set that option
yourself, which centers those strata at the grand mean;
...
value, and `write_dta()` turns those factors into Stata value labels, so
the codes arrive already documented.
