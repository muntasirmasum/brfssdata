# Articles

### All vignettes

- [Age-adjusted
  prevalence](https://muntasirmasum.github.io/brfssdata/articles/age-adjustment.md):
- [Getting started with
  brfssdata](https://muntasirmasum.github.io/brfssdata/articles/brfssdata.md):
- [The
  datasets](https://muntasirmasum.github.io/brfssdata/articles/datasets.md):
- [Merging BRFSS with external
  data](https://muntasirmasum.github.io/brfssdata/articles/external-data.md):
- [Survey-weighted logistic regression with
  BRFSS](https://muntasirmasum.github.io/brfssdata/articles/logistic-regression.md):
- [Using the data outside
  R](https://muntasirmasum.github.io/brfssdata/articles/outside-r.md):
- [Survey design in
  BRFSS](https://muntasirmasum.github.io/brfssdata/articles/survey-design.md):
- [Validating against CDC's published
  estimates](https://muntasirmasum.github.io/brfssdata/articles/validation.md):
- [Value labels and missing
  codes](https://muntasirmasum.github.io/brfssdata/articles/value-labels-missing-codes.md):
