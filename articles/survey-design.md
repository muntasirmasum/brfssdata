...
sample was actually drawn. Skipping either changes the answer itself,
and the standard error attached to it means nothing.
...
[`brfss_design()`](https://muntasirmasum.github.io/brfssdata/reference/brfss_design.md)
leaves it out. Estimates, standard errors, and degrees of freedom are
unchanged, and `survey` is spared a cluster factor with one level per
respondent, which is the difference between a mean that returns in about
a second and one that takes over a minute.
...
also assumes each pooled year covers the same states; participation does
drift (states join, skip a year, return), and when the requested years
differ that way
[`brfss_design()`](https://muntasirmasum.github.io/brfssdata/reference/brfss_design.md)
warns with the state codes involved, because a pooled total then
averages over changing coverage.
...
these three years and the intervals do not overlap, so it is not
sampling noise, but neither is it necessarily a change in how people
feel. The 2011 guard covers the break CDC calls disqualifying; it says
nothing about two years on the same side of it. Raking margins, which
states field the survey, and the conditions collection happens under all
move from year to year, and the early 2020s moved more than most. CDC
ships a comparability document with each annual release, and a shift
this size belongs there before it belongs in a trend line.
