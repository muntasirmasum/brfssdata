...
> releases are being prepared. `brfss_years()` reports which survey years
> are published at any moment.
...
Downloads are cached under `brfss_cache_dir()` (per
`tools::R_user_dir()`), so everything after the first pull works offline.
...
[`data-raw/`](data-raw/) and every artifact is checksummed.
