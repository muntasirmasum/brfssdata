...
Weights, strata, and primary sampling units are already applied, with
the right weight for the survey era; codes CDC uses for “don’t know” and
“refused” arrive as `NA` (that is the `na = TRUE` default on
[`brfss_design()`](https://muntasirmasum.github.io/brfssdata/reference/brfss_design.md)),
so the proportions cover substantive answers. Add `labels = TRUE` if you
want the factor levels spelled out.
